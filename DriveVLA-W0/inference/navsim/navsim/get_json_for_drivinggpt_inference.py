...
# uncomment for navtest
# SPLIT = "test"  # ["mini", "test", "trainval"]
# FILTER = "navtest"

# uncomment for navtrain
...
scene_loader = SceneLoader(
    ...
    SensorConfig(cam_f0=include, cam_l0=False, cam_l1=False, cam_l2=False, cam_r0=False, cam_r1=False, cam_r2=False, cam_b0=False, lidar_pc=False),
)

# 示例任务函数，处理列表中的单个元素
def process_element(token):
    # 在这里实现具体的处理逻辑
    print(f"Processing: {token}")
    
    scene_list = scene_loader.scene_frames_dicts[token]
    image_paths = []
    for scene in scene_list:
        image_paths.append(scene["cams"]["CAM_F0"]["data_path"])
    scene = scene_loader.get_scene_from_token(token)
    scene_dict = {}
    scene_dict["images"] = image_paths
    scene_dict["poses"] = []
    ...
    # uncommment for navtrain
    SAVE_DIR = "/mnt/vdb1/hongxiao.yu/data/all/navtrain_json/"
    # uncomment for navtest
    # SAVE_DIR = "/mnt/vdb1/hongxiao.yu/data/all/navtest_json/"
...
tokens = scene_loader.tokens  # get all current scene tokens, each corresponds to a dict list


# 创建进程池
with multiprocessing.Pool(processes=16) as pool:
    # 使用进程池处理元素列表
    results = pool.map(process_element, tokens)
...
#     SAVE_DIR = "/mnt/vdb1/hongxiao.yu/data/all/navsim_train_vanilla_json_debug/"
#     FILE_NAME_PATTERN = f"{scene.scene_metadata.log_name}_{token}.json"
