...
class TrainDataset(Dataset):
    def __init__(self, nuscenes_path, nuscenes_json_path, condition_frames=9, downsample_fps=3, downsample_size=16, h=256, w=512):
        ...
        for video_keys in nuscenes_keys:
            ...
            if len(token_path_poses) <= 75:
                continue
            
            for start_idx in range(0, len(token_path_poses) - 75, 15):  # 每 10 帧作为新的起点
                tmp_token_path = []
                tmp_pose = []
                for i in range(start_idx, start_idx + 75):  # 取 75 帧数据
                    token_path_pose = token_path_poses[i]
        ...
        print("Length of data:", len(self.token_path_data))
