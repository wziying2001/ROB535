...
from custom_mmdet3d.core.bbox import BaseInstance3DBoxes
from custom_mmdet3d.core.points import BasePoints
from mmdet.datasets.builder import PIPELINES
from mmdet.datasets.pipelines import to_tensor
from custom_mmdet3d.datasets.pipelines import DefaultFormatBundle3D
...
class CustomDefaultFormatBundle3D(DefaultFormatBundle3D):
    ...
    def __call__(self, results):
        ...
        if 'semantic_img' in results:
            if isinstance(results['semantic_img'], list):
                # process multiple imgs in single frame
                imgs = [img.transpose(2, 0, 1) for img in results['semantic_img']]
                imgs = np.ascontiguousarray(np.stack(imgs, axis=0))
                results['semantic_img'] = DC(to_tensor(imgs), stack=True)
            else:
                img = np.ascontiguousarray(results['semantic_img'].transpose(2, 0, 1))
                results['semantic_img'] = DC(to_tensor(img), stack=True)
        
        if 'flow_img' in results:
            if isinstance(results['flow_img'], list):
                # process multiple imgs in single frame
                imgs = [img.transpose(2, 0, 1) for img in results['flow_img']]
                imgs = np.ascontiguousarray(np.stack(imgs, axis=0))
                results['flow_img'] = DC(to_tensor(imgs), stack=True)
            else:
                img = np.ascontiguousarray(results['flow_img'].transpose(2, 0, 1))
                results['flow_img'] = DC(to_tensor(img), stack=True)
