    ...
    def _pad_img(self, results):
        ...
        if self.size is not None:
            ...
            if 'semantic_img' in results.keys():
                padded_semantic_img = [mmcv.impad_to_multiple(
                    img, self.size_divisor, pad_val=self.pad_val) for img in results['semantic_img']]
                results['semantic_img'] = padded_semantic_img
            
            if 'flow_img' in results.keys():
                padded_flow_img = [mmcv.impad_to_multiple(
                    img, self.size_divisor, pad_val=self.pad_val) for img in results['flow_img']]
                results['flow_img'] = padded_flow_img
...
class NormalizeMultiviewImage(object):
    ...
    def __call__(self, results):
        ...
        if 'semantic_img' in results.keys():
            results['semantic_img'] = [mmcv.imnormalize(img, self.mean, self.std, False) for img in results['semantic_img']]
...
@PIPELINES.register_module()
class NormalizeFlowImage(object):
    """Normalize the image.
    Added key is "img_norm_cfg".
    Args:
        mean (sequence): Mean values of 3 channels.
    ...
    def __call__(self, results):
        ...
        flow_img_list = []
        for img in results['flow_img']:
            img = img.astype(np.float32)
            img = (img - self.mean) / self.std
            flow_img_list.append(img)
        results['flow_img'] = flow_img_list
        return results
...
class PhotoMetricDistortionMultiViewImage:
    ...
    def generate_random_bools(self, seed):
        local_random = np.random.RandomState(seed)
        return [bool(local_random.randint(2)) for _ in range(self.num_augs)]
    ...
    def __call__(self, results):
        ...
        imgs = results['img']
        new_imgs = []
        if 'aug_random_int' in results.keys():
            random_bools_list = self.generate_random_bools(results['aug_random_int'])
        else:
            random_bools_list = [random.randint(2) for _ in range(len(self.num_augs))]
        ...
        for img in imgs:
            ...
            mode = random_bools_list[1]
            if mode == 1:
                if random_bools_list[2]:
            ...
            # random saturation
            if random_bools_list[3]:
                img[..., 1] *= random.uniform(self.saturation_lower,
                                            self.saturation_upper)

            # random hue
            if random_bools_list[4]:
            ...
            # random contrast
            if mode == 0:
                if random_bools_list[5]:
                    alpha = random.uniform(self.contrast_lower,
                                        self.contrast_upper)
                    img *= alpha

            # randomly swap channels
            if random_bools_list[6]:
...
class CustomCollect3D(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                            ...
                            'can_bus', 'lidar2cam', 'lidar2global', 'lidar2ego_translation'
                            )):
        self.keys = keys
...
class RandomScaleImageMultiViewImage(object):
    ...
    def __call__(self, results):
        ...
        if 'flow_img' in results.keys():
            results['flow_img'] = [mmcv.imresize(img, (x_size[0], y_size[0]), return_scale=False) for idx, img in
                                       enumerate(results['flow_img'])]
