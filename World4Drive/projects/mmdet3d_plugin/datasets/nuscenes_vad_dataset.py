...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.use_video_test_mode = use_video_test_mode
        self.load_gt_map = load_gt_map

        # self.load_gt_map = True
        # self.filter_empty_gt = True
        # # self.is_vis_on_test = True
        # print('load_gt_map is True for training VAD.')
    ...
    def _set_sequence_group_flag(self):
        ...
        """
        # way 1
        # num_elements = len(self.data_infos)
        # flag = []
        # seq_length = 40 // self.seq_split_num
        # for i in range(num_elements):
        #     # 每 seq_length 个元素，值增加 1
        #     value = i // seq_length
        #     flag.append(value)

        # # 确保最后一个元素至少出现 seq_length 次
        # # mod_value = num_elements % self.seq_length
        # # if mod_value != 0:
        # #     for j in range(mod_value):
        # #         flag[-j-1] = flag[-j-1] - 1
        ...
        if self.seq_split_num != 1:
            if self.seq_split_num == 'all':
                ...
                for curr_flag in range(len(bin_counts)):
                    # split_size = round(40 / self.seq_split_num)
                    split_size = round(bin_counts[curr_flag] / self.seq_split_num)
                    range_list = list(range(0, bin_counts[curr_flag], split_size)) + [bin_counts[curr_flag]]

                    # 确保列表中的元素数量是 self.seq_split_num + 1
                    # 如果不是，可能需要调整最后一个元素
                    if len(range_list) > self.seq_split_num + 1:
                        range_list = range_list[:self.seq_split_num] + [bin_counts[curr_flag]]
                    elif len(range_list) < self.seq_split_num + 1:
                        range_list.extend([bin_counts[curr_flag]] * (self.seq_split_num + 1 - len(range_list)))

                    curr_sequence_length = np.array(list(range_list))
    ...
    def prepare_train_data(self, index):
        ...
        aug_random_int = np.random.randint(0, 100000)
        frame_idx = input_dict['frame_idx']
        scene_token = input_dict['scene_token']
        input_dict['aug_random_int'] = aug_random_int
        ...
        for my_index, i in enumerate(prev_indexs_list):
            if self.interval_2frames and my_index != (len(prev_indexs_list) - 1):
                continue
    ...
    def prepare_test_data_video(self, index):
        """Prepare data for testing as video sequence.

        Args:
            index (int): Index for accessing the target data.

        Returns:
            dict: Testing data dict of the corresponding index and previous frames.
        """
        data_queue = []
        # set_trace()
        prev_indexs_list = list(range(index-self.queue_length, index))
    ...
    def union2one(self, queue):
        ...
        for i, each in enumerate(queue):
            ...
            metas_map[i] = each['img_metas'][0].data if isinstance(each['img_metas'], list) else each['img_metas'].data

            # include ego info into img_metas
            metas_map[i].update({
                'ego_fut_trajs': each['ego_fut_trajs'].data if not isinstance(each['ego_fut_trajs'], list) else each['ego_fut_trajs'][0].data,
                'ego_fut_masks': each['ego_fut_masks'].data if not isinstance(each['ego_fut_masks'], list) else each['ego_fut_masks'][0].data,
                'ego_fut_cmd': each['ego_fut_cmd'].data if not isinstance(each['ego_fut_cmd'], list) else each['ego_fut_cmd'][0].data,
                'ego_his_trajs': each['ego_his_trajs'].data if not isinstance(each['ego_his_trajs'], list) else each['ego_his_trajs'][0].data,
            })
        ...
        #only update image, semantic_img, img_metas, others are only using last frame
        queue[-1]['img'] = DC(torch.stack(imgs_list),
                            cpu_only=False, stack=True)
        if len(sem_imgs_list) > 0:
            queue[-1]['semantic_img'] = DC(torch.stack(sem_imgs_list),
                                cpu_only=False, stack=True)
    ...
    def evaluate(self,
        ...
        metric_dict['plan_L2_avg'] = (metric_dict['plan_L2_1s'] + metric_dict['plan_L2_2s'] + metric_dict['plan_L2_3s']) / 3
        metric_dict['plan_obj_box_col_avg'] = (metric_dict['plan_obj_box_col_1s'] + metric_dict['plan_obj_box_col_2s'] + metric_dict['plan_obj_box_col_3s']) / 3
        #eval planning only
        return metric_dict
