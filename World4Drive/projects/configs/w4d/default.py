...
_dim_ = 256 # origin 256
_pos_dim_ = _dim_//2
_ffn_dim_ = _dim_*2
_num_levels_ = 1
bev_h_ = 100
bev_w_ = 100
queue_length = 4 # skip two frames origin 4
total_epochs = 24 # 24
...
model = dict(
    ...
    hidden_channel=256, # origin
    wm_loss_weight=0.2,
    img_backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
        out_indices=(3,), # origin 3
        frozen_stages=1,
    ...
    pts_bbox_head=dict(
        type='SimpleWayDecoderHead',
        num_proposals=6,
        num_views=6,
        hidden_channel=256, # origin 256
        num_heads=8,
        dropout=0.0,
        use_wm=True,
        num_traj_modal=3,
...
train_pipeline = [
    ...
    dict(type='LoadMultiDepthFromFiles',to_float32=True),
    dict(type='LoadMultiMaskFromFiles',to_float32=True),
        ...
        transforms=[
            ...
            dict(type='CustomCollect3D',\
                 keys=['points', 'gt_bboxes_3d', 'gt_labels_3d', 'img', 'fut_valid_flag',
                       ...
                       'ego_lcf_feat', 'gt_attr_labels', 'depth'])])
]
